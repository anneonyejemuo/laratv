...
class CategoriesModel extends CI_Model
{
    ...
    {
        $sql = "SELECT title, url, id_relation, image FROM 2d_categories WHERE id = ?";
    ...
    public function addCategorie($postTitle, $postURL, $postParentCat)
    {
        ...
        if($query->num_rows() > 0) {
            $msg = alert($this->lang->line('This category already exists'), 'danger');
        } else {
            ...
            $msg = alert($this->lang->line('The category was created').' <a href="/dashboard/categories/edit/'.$this->db->insert_id().'">'.$this->lang->line('Edit it').'</a> !');
        }
        return $msg;
    }
    ...
    {
        $sql = "UPDATE 2d_categories SET image = ? WHERE id = ?";
        $this->db->query($sql, array($image, $idCategory));
    }

    public function delCategorie($idCategory)
    {
        $sql = "SELECT ga.id AS id_videos FROM 2d_categories ca, 2d_videos ga  WHERE ((ca.id = ?) AND (ca.id = ga.id_category))";
        $query = $this->db->query($sql, array($idCategory));
        if($result = $query->row()) {
            return alert($this->lang->line('Warning! This category still contains videos. You must change the category of these videos before deleting.'), 'danger');
        } else {
